...
interface SchemaDefinitionContentProps {
  data: AppData;
...
export default function SchemaDefinitionContent({
  data,
  onUpdate,
  errors,
  isLoading,
  setIsLoading
}: SchemaDefinitionContentProps) {
  ...
  function extractFieldDefs(sql: string): Array<{ name: string; type: string }> {
    ...
    const startIdx = createTableMatch.index! + createTableMatch[0].length - 1;
    let depth = 1;
    let endIdx = startIdx;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Upload SQL Schema File Section */}
      <div>
      ...
      {/* Field Mapping UI (for Cleared Checks) */}
      {showMappingUI && (
        <div className="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
